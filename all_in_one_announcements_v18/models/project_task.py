...
class ProjectTask(models.Model):
    ...
    def task_assigned(self):
        """
        Get assigned tasks and send announcements if not already sent.
        """
        tasks = self.search([('stage_id.name', 'ilike', 'In Progress')])
        purchase_orders = self.env['purchase.order'].search(
            [('state', '=', 'purchase')])
        sale_orders = self.env['sale.order'].search(
            [('state', '=', 'sale')])
        crm_lead = self.env['crm.lead'].search([('stage_id.name', 'ilike', 'Won')])
        context = {}
        result = []
        mail_param = self.env['ir.config_parameter'].sudo()
        for task in tasks:
            name = task.project_id.name or 'Undefined Project'
            if name in context:
            ...
            if not task.is_announcement_send:
                if mail_param.get_param('all_in_one_announcements.is_email'):
                    for user_id in task.user_ids:
                        mail_content = f"Hello {task.name}, you have a pending task. Please check."
                        main_content = {
                            'email_from': self.env.user.email or self.env.company.email,
                            'email_to': user_id.email or 'admin@example.com', # fallback or use a setting
