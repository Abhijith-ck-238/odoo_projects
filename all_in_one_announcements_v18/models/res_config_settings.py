...
class ResConfigSettings(models.TransientModel):
    ...
    def email_send(self):
        ...
        task_recs = self.env['project.task'].search([('stage_id.name', 'ilike', 'In Progress')])
        tasks = [{
            'name': rec.name,
        ...
        crm_recs = self.env['crm.lead'].search([('stage_id.name', 'ilike', 'Won')])
        crm_leads = [{
            'name': rec.name,
            'url': f"{base_url}/web#id={rec.id}&model=crm.lead&view_type=form"
        } for rec in crm_recs]

        is_manager = group_manager in self.env.user.groups_id
        email_to = ','.join(group_manager.users.mapped('email') or group_manager.users.mapped('login'))
