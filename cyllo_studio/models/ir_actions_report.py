...
class IrActionsReport(models.Model):
    ...
    def _render_qweb_html(self, report_ref, docids, data=None):
        ...
        print('docids', docids)
        print('_render_qweb_html', self._render_template(report.report_name, data))
    ...
    def get_values(self):
        ...
        for tes in reports:
            print(123213123,tes)
            print(1232131231,tes.report_name)
            qweb_view = self.env['ir.ui.view'].search([('xml_id', '=', tes.report_name)])
            print("qwerty", qweb_view)
        return [{'id': rec.id, 'name': rec.name, 'model_id':rec.model_id, 'model':rec.model, 'report_name': rec.report_name} for rec in reports]

    @api.model
    def get_qweb(self, data):
        print("Argsgs", data['report_name'])
        # action_ref = self.env.ref('base.action_ui_view')
        # if not action_ref or len(data['report_name'].split('.')) < 2:
        #     return False
        # action_data = action_ref.read()[0]
        # action_data['domain'] = [('name', 'ilike',data['report_name'].split('.')[1]), ('type', '=', 'qweb')]
        qweb_code = self.env['ir.ui.view'].search([('name', 'ilike', data['report_name'].split('.')[1]), ('type', '=', 'qweb')])
        for i in qweb_code:
            print(i.arch)
