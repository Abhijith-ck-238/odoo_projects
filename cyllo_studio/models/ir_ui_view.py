        ...
        for node in tree.xpath('//*[@groups]'):
            ...
            if node.tag == 't' and node.get('postprocess_added'):
                for child in reversed(node):
                    ...
                    # child.set("groups", combined_groups_str) # ToDO Need to find better way for handle default group fields.
...
class Model(models.AbstractModel):
    ...
    def get_view(self, view_id=None, view_type='form', **options):
        ...
        if view_type in views:
            ...
            # xml_string = re.sub(r'<t[^>]*>.*?</t>', '', xml_string, flags=re.DOTALL)
        ...
        #         '\t', '')
        # if view_type == 'form':
        # print("MineResult", result['arch'])
        return result

    # def _get_view(self, view_id=None, view_type='form', **options):
    #     arch, view = super()._get_view(view_id, view_type, **options)
    #     studio = request.session.studio
    #     is_not_studio_debug = '1' not in studio if studio else False
    #
    #     if is_not_studio_debug:
    #         return arch, view
    #     else:
    #         account_node = next(iter(arch.xpath('//field[@name="account_id"]')), None)
    #         if account_node is not None:
    #             parent = account_node.getparent()
    #             parent.remove(account_node)
    #         return arch, view