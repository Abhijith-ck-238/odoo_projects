...
export class MenuCreationDialog extends Component {
    setup() {
        ...
        console.log("qwefdwsdv",this)
    }
//    _onActionTypeChange(ev) {
//        this.state.ActionType = ev.target.value
//        this.props.Menu.actionModel = ev.target.value
//    }
    onUpdate(resId, self) {
        self.state.resId = resId
        console.log("self.state.resId",self.state.resId)
    }
//    onUpdateMenu(resId, self) {
//        self.state.menuResId = resId
//    }
    onInputModelName(ev) {
        console.log("inputmodelname")
        var input_type = document.getElementById('model_name');
        console.log('input_type',input_type)
        console.log('input_typename',this.state)
        if (input_type.value == '') {
    ...
    async OnclickConfirmDialogue(ev){
        console.log("111")
        console.log("wesaxds",document.querySelector('.menu-name').value)
        if (document.querySelector('.menu-name').value) {
            var data = this.rpc("/cyllo_studio/menuitem/confirm", {
                method: 'menu_confirm',
                args: [],
                kwargs: {
...
export class MenuConfigurationDialog extends Component {
    async setup() {
        ...
        this.state = useState({
            ...
            actionID: this.props.Menu.actionID,
            ActionType: this.props.Menu.actionModel,
            SiblingPosition: {},
            ParentChange: false,
            menuDetails: false,
...
//            for (let key in this.props.ParentMenus) {
//                if (this.props.ParentMenus[key].id == ParentMenus) {
//                    this.state.ParentChange = true
//                    this.props.ParentMenu.childrenTree = this.props.ParentMenus[key].childrenTree
//                    if (! this.props.ParentMenus[key].childrenTree) {
//                        this.props.Siblings = this.props.ParentMenus
//                        this.state.ChildParentMenu = this.props.ParentMenus
//                    }
//                    this.props.ParentMenu = this.props.ParentMenus[key]
//                }
//            }
        } else {
            console.log("its a sibilnng")
    ...
    _onActionTypeChange(ev) {
        this.state.ActionType = ev.target.value
        this.props.Menu.actionModel = ev.target.value
        console.log("qwasd123",this.state.ActionType)
        console.log("qwasd1234",this.props.Menu.actionModel)
    }
    async OnclickConfirmDialogue(ev){
        console.log("1112")
        console.log("asdfqwer223",this)
        var MenuPosition = this.state.SiblingPosition
        console.log("asdfqwer22001",MenuPosition)
        if (Object.entries(MenuPosition).length <= 1) {
            var elements = document.querySelectorAll('.SiblingMenu');
            elements.forEach(function(element, index) {
                var Fields = element.attributes.value ? element.attributes.value.value : [null]
                console.log("asdfqwer22002",Fields)
                MenuPosition[index] =  Fields;
            })
        }
        console.log("asdfqwer22003")
    ...
    //MenuConfigurationDialog.components = { Dialog, MultiRecordSelector, RecordSelector, CheckBox, CylloRecordSelector };
...
export class ParentMenuConfigurationDialog extends Component {
    ...
    async OnclickConfirmDialogueParentMenu(ev){
        console.log("1113")
        let iconImage = ''
        console.log("onclickconfirmdialog",this)
        console.log("onclickconfirmdialog",this.props.ParentMenu.webIcon)
        console.log("onclickconfirmdialog",this.state.iconDefault)
