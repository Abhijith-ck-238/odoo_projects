...
class VehicleRental(models.Model):
    ...
    state = fields.Selection(
        ...
        help="Status of renting the vehicle, Which is th status of the "
             "vehicle rented?")
    vehicle_paid_bool = fields.Boolean(string='Paid Bool', default=False,
                                       copy=False,help='Indicates whether the vehicle payment has been completed.')
    ...
    def action_return(self):
        ...
        if self.start_date > date.today():
            raise ValidationError(_("Return is not allowed before the rental start date."))
        else:
            self.state = 'return'
    ...
    def action_register_payment(self):
        ...
        view_id = self.env.ref(
            'agriculture_management_odoo.vehicle_register_payment_wizard_'
            'view_form').id
        return {
