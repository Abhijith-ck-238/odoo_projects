...
class PestDetail(models.Model):
    ...
    pest_description = fields.Text(string='Pest Description', tracking=True,
                                   help="Brief description about the "
                                        "pesticide")
    ...
    @api.constrains('pest_cost', 'pest_quantity')
    def _check_positive_values(self):
        """Constraint to ensure pest cost and quantity are greater than zero"""
        for record in self:
            if record.pest_cost <= 0:
                raise ValidationError(_('Pesticide cost must be greater than zero.'))
