...
class AnimalRental(models.Model):
    ...
    crop requests. It provides a structured way to manage information
    related to the rental of animals for agricultural purposes. """
    _name = 'animal.rental'
    ...
    animal_id = fields.Many2one('animal.detail', string='Animal',
                                ...
                                required=True, tracking=True,domain=[('state', '=', 'available')])
    ...
    state = fields.Selection(
        ...
        help="Status of rental animals.")
    animal_paid_bool = fields.Boolean(string='Paid Bool', default=False,
                                      copy=False,help='Indicates whether the animal payment has been completed.'
    )
    ...
    def action_return(self):
        ...
        if self.start_date > date.today():
            raise ValidationError(
                _("Return is not allowed before the rental start date."))
        else:
            self.state = 'return'
    ...
    def action_register_payment(self):
        ...
        view_id = self.env.ref(
            'agriculture_management_odoo.animal_register_payment_wizard_view_'
            'form').id
        return {
