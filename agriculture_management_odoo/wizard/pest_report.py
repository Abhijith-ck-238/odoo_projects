    ...
    def action_pdf_report(self):
        ...
        if self.date_from and not self.date_to:
            if self.date_from > today:
                ...
                ret = ret + (
                        " WHERE pest_request.request_date >= '" + str(
                         self.date_from) +
                        "' AND pest_request.request_date <= '" + str(
                         today) + "'"
                )
        ...
        if record:
            return self.env.ref(
                'agriculture_management_odoo.pest_request_report_action'
                '').report_action(
                self, data=data)
        else:
            raise ValidationError(
                _('No Data Available in this period'))
