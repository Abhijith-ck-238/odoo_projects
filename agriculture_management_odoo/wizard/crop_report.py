...
class CropReport(models.TransientModel):
    ...
    def action_pdf_report(self):
        ...
        ret = """
        SELECT crop_request.ref, res_partner.name,
               seed_detail.name, crop_request.request_date,
               crop_request.state, location_detail.location_name
        FROM crop_request
        INNER JOIN farmer_detail
            ON crop_request.farmer_id = farmer_detail.id
        INNER JOIN res_partner
            ON farmer_detail.farmer_id = res_partner.id
        INNER JOIN seed_detail
            ON crop_request.seed_id = seed_detail.id
        INNER JOIN location_detail
            ON crop_request.location_id = location_detail.id
        """
        ...
        if self.date_from and not self.date_to:
            if self.date_from > today:
                ...
                ret = ret + (
                        " WHERE crop_request.request_date >= '" + str(
                         self.date_from) +
                        "' AND crop_request.request_date <= '" + str(
                         today) + "'"
                )
        ...
        if not record:
            raise UserError(_('No data found for the selected date range.'))
        data = {
            'form': self.read()[0],
        ...
        return (self.env.ref(
            'agriculture_management_odoo.action_crop_request_report'
            '').report_action(
            self, data=data))
