    ...
    def action_repair_report(self):
        ...
        if 'from_date' in data and 'to_date' in data:
            vals = self.env['machine.repair'].search([
                ('repair_req_date', '>=', data['from_date']),
                ('repair_req_date', '<=', data['to_date'])])
        else:
            vals = self.env['machine.repair'].search([])
        if vals:
            return self.env.ref('base_machine_repair_management.action_repair_report').report_action(self, data=data)
        else:
            return {
                'type': 'ir.actions.client',
                'tag': 'display_notification',
                'params': {
                    'message': 'There is no repair request...',
                    'type': 'success',
                    'sticky': False,
