...
class CustomGenPurchase(models.TransientModel):
    _inherit = "gen.purchase"

    new_import_option = fields.Selection([('xls', 'XLS File')], string='Select', default='xls')
    new_import_prod_option = fields.Selection([('code', 'Code'), ('barcode', 'Barcode'),('name', 'Name')],
    ...
    def make_purchase_line(self, values, pur_id):
        ...
        if self.new_import_prod_option == 'barcode':
            product_search = product_obj.search([('barcode', '=', values['product'])])
        elif self.new_import_prod_option == 'code':
        ...
        if product_search:
            ...
            if self.new_import_prod_option == 'name':
