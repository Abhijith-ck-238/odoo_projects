...
class Rec_Api():
    ...
    def __init__(self):
        self.username ="admin" 
        self.password ="admin"
        self.db ="Alitkan_Test" 
        self.url ="http://173.249.59.179:8069" 
    ...
    def create_record_candidate(self,fields,module="hr.candidate",op_type="create"):
...
#            else:
            try:
                ...
                candidate = obj.create_record_candidate(fields={'partner_name': data['name']})
                data.update({'candidate_id': candidate})
                name = data['name']

                data.pop('name')
                res=obj.create_record(fields=data)
                s_res=obj.search_record(domain=[["partner_name","=", name]])
                if s_res != []:
                    ...
                    failure_massage = "Error Code: 1243 => " + str(res)
