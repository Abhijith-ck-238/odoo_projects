...
class CustomCrossoveredBudget(models.Model):
    ...
    def send_budget_deadline_notification(self):
        ...
        for rec in res:
            users = self.env['ir.config_parameter'].sudo().get_param('custom_expense.budget_reminder_user_ids')
            if users == []:
    ...
    def compute_remaining_budget(self):
        for rec in self:
            ...
            for line in rec.budget_line_ids:
                tot = tot + line.planned_amount
            budget_line_id = self.env['budget.line'].search([
                ('analytic_account_id', '!=', False),
                ('budget_analytic_id', '=', rec.id)], limit=1)
            if budget_line_id:
                if budget_line_id.analytic_account_id.line_ids:
                    rec.remaining_budget = budget_line_id.analytic_account_id.sudo().remaining_budget_amount
                else:
    ...
    def compute_is_dm(self):
        ...
        for rec in self:
            ...
            if res.budget_manager.user_id.id == self._uid:
                rec.is_dm = True
            else:
                rec.is_dm = False
    ...
    def action_budget_validate(self):
        for budget_line in self.budget_line_ids:
            if budget_line.amount_to_extend == 0.0:
                ...
                budget_line.planned_amount = budget_line.planned_amount + budget_line.amount_to_extend
    ...
    def action_check_budget(self):
        has_planned_amount = any(
            line.planned_amount == 0.0 for line in self.budget_line_ids)
    ...
    def action_approve_ac(self):
        ...
        if total != 0.0:
            ...
            for line in self.budget_line_ids:
                if line.amount_to_extend:
                    line.planned_amount += line.amount_to_extend
                    line.amount_to_extend = 0
