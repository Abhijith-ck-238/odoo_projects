...
class LetterTicket(models.Model):
    _name = "letter.ticket"
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = "Letter Ticket"

    def _get_default_stage_id(self):
        return self.env['letter.stage'].search([], order='sequence', limit=1)

    name = fields.Char(string="Ticket Description", tracking=True)
    stage_id = fields.Many2one('letter.stage',string="Stage", copy=False,
                               default=_get_default_stage_id, group_expand='_read_group_stage_ids', tracking=True, store=True)
    letter_type_id = fields.Many2one('letter.type', string='letter Type', required=True)
    ...
    modality_id = fields.Many2one('contract.modality', string="Modality", required=True)
    approval_letter = fields.Many2many('ir.attachment', string="Approval Letter", relation="rel_attachment_approval_letter")

    @api.model
    def _read_group_stage_ids(self, stages, domain, order=None):
        return self.env['letter.stage'].search([], order=order or "sequence")

    def write(self, vals):
        if vals.get('stage_id'):
            new_stage_id = self.env["letter.stage"].browse(vals.get('stage_id'))
            if self.stage_id:
                if new_stage_id:
                    ...
                    if user.id in new_stage_id.move_from_user_ids.ids:
                        self.env['mail.activity'].sudo().create({
                            ...
                            'activity_type_id': self.env.ref(
                                'custom_letters.mail_activity_letter_ticket').id,
                        })
            else:
            ...
            if new_stage_id.require_approval_letter and not self.approval_letter:
                raise UserError(_("You need to upload Approval Letter before moving the task to this stage."))

        return super(LetterTicket, self).write(vals)

    @api.model
    def create(self, vals):
        res = super(LetterTicket, self).create(vals)
        ...
        if vals.get('stage_id'):
            new_stage_id = self.env["letter.stage"].browse(
                vals.get('stage_id'))
            if new_stage_id.send_activity_for_modality_manager:
                users = self.modality_id.user_ids
                for user in users:
