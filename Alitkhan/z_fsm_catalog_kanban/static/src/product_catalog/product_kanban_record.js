...
console.log("ProductCatalogKanbanRecord")
...
patch(ProductCatalogKanbanRecord.prototype, {
    ...
    customUpdateQuantity(quantity){
        if (this.props.record.context.fsm_task_id) {
            return this.updateQuantityBase(quantity);
        }
        else{
            if (this.env.orderResModel !== "sale.order" || this.productCatalogData.productType == "service") {
                this.updateQuantityBase(...arguments);
                }
            else if (
                ...
                this.updateQuantityBase(Math.max(quantity, this.productCatalogData.deliveredQty));
            }
        }
    },
