...
class ProductController extends FormController {
    setup() {
        ...
        window.addEventListener("click", async (ev) => {
            if (ev.target.classList.contains('print_product_rfid_data')) {
                await this._printRfidTag(ev)
            }
        })
    }
    async _printRfidTag(ev) {
        var self = this
        if (owl.status(this) === "destroyed"){
            return
        }
        let res = await this.orm.call("rfid.tag.epc", "create_rfid_tag", [[]], {product_id: this.model.root.data.id})

        let row_zpl = ["^XA^RFW,A^FD" + res + "^FS^FO30,50^A0N,44,33^FD" +
        self.model.root.data.display_name + "^FS^FO100,100^A0N,44,33^FD" + self.model.root.data.default_code + "^FS"]
        if (self.model.root.data.barcode !== false) {
            row_zpl += ["^FO100,150^BY3^BCN,100,Y,N,N^FD" + self.model.root.data.barcode + "^FS"]
        }
        row_zpl += ["^XZ"]
        let printername = session.rfid_printer_name
        if (printername !== false) {
            ...
            this.notification.add(
                _t("There is no specific printers to print RFID tags. Please add a printer in configuration settings."),
                {type: "warning"}
            );
        }
...
registry.category("views").add("rfid_tag_print", {
    ...formView,
    Controller: ProductController,
});


class StockLotController extends FormController {
    setup() {
        super.setup();
        this.notification = useService("notification");
        window.addEventListener("click", async (ev) => {
            if (ev.target.classList.contains('print_lot_data')) {
                await this._printRfidTag(ev)
            }
        })
    }
    ...
    async _printRfidTag(ev) {
        ...
        const product_id = this.model.root.data.product_id[0]
        const lot_id = this.model.root.resId
        let res = await this.orm.call("rfid.tag.epc", "create_lot_rfid_tag", [[]], {product_id,lot_id})
        let row_zpl = ["^XA^RFW,A^FD" + res.sequence + "^FS^FO30,50^A0N,44,33^FD" +
                                res.product_name + "^FS^FO100,100^A0N,44,33^FD" +
                                res.default_code + "^FS^FO100,150^A0N,44,33^FD" +
                                self.model.root.data.name + "^FS"]
        if (res.barcode !== false) {
            row_zpl += ["^FO100,200^BY3^BCN,100,Y,N,N^FD" + res.barcode + "^FS"]
        }
        row_zpl += ["^XZ"]
        let printerName = session.rfid_printer_name
        if (printerName !== false) {
...
registry.category("views").add("rfid_lot_tag_print", {
    ...formView,
    Controller: StockLotController,
});

class StockPickingtController extends FormController {
    setup() {
        super.setup();
        this.notification = useService("notification");
        window.addEventListener("click", async (ev) => {
            if (ev.target.classList.contains('sent_picking_label_to_printer')) {
                await this._printRfidTag(ev)
            }
        })
    }
    ...
    async _printRfidTag(ev) {
        ...
        const line_ids = this.model.root.data.move_ids_without_package.resIds
        let res = await this.orm.call("rfid.tag.epc", "create_picking_rfid_tag", [[]], {line_ids})
        let row_zpl = [res]
        let printerName = session.rfid_printer_name
        if (printerName !== false) {
            if (!qz.websocket.isActive()) {
...
registry.category("views").add("rfid_picking_tag_print", {
    ...formView,
    Controller: StockPickingtController,
});
