...
class CustomCrossoveredBudget(models.Model):
        ...
        for rec in res:
            ...
            if date == datetime.today().date():
                notification_ids = [((0, 0, {
                    ...
                    'notification_type': 'inbox'})) for p in users_record]
                ...
                message = _(
                    ...
                    "actions <a href=# data-oe-model=budget.analytic data-oe-id=%d>%s</a> will end within 7 days"
                ) % (rec.id, rec.name)
                ...
                message_body = _(
                    "Dear %s,\n This email to remind you that this deadline will be met on for your information and "
                    "actions <a href=# data-oe-model=budget.analytic data-oe-id=%d>%s</a> will end within 7 days"
                ) % (rec.user_id.name, rec.id, rec.name)
                rec.message_post(
                    body=message_body,
                    message_type='comment',
                    subtype='mail.mt_comment',
    ...
    def compute_remaining_budget(self):
        for rec in self:
            ...
            for line in rec.budget_line_ids:
                tot = tot + line.budget_amount
            budget_line_id = self.env['budget.line'].search([
                ('x_plan6154_id', '!=', False),
                ('budget_analytic_id', '=', rec.id)], limit=1)
            if budget_line_id:
                if budget_line_id.x_plan6154_id.line_ids:
                    rec.remaining_budget = budget_line_id.x_plan6154_id.sudo().remaining_budget_amount
                else:
                    rec.remaining_budget = tot
            else:
                rec.remaining_budget = tot
    ...
    def _send_budget_notification(self, subject, message, user_name=False,user_email=False):
        action_id = self.env.ref(
            "custom_expense_budget_approval.hr_expense_actions_my_budget").id
        ...
        email_to = user_email if user_email else self.user_id.email
        mail_values = {
            'email_to': email_to,
    ...
    def action_budget_confirm(self):
        res = super(CustomCrossoveredBudget, self).action_budget_confirm()
    ...
    def action_check_budget(self):
        ...
        approver_user = self.env["ir.config_parameter"].sudo().get_param(
            "custom_expense.budget_check_approver") #Farah alaa
        if approver_user:
            ...
            if accounting_users:
                ...
                for user_id in accounting_user_ids:
                    ...
                    self._send_budget_notification(
                        ...
                        user_email=res_user.login
                    )

                self.write({"expense_state": "ac_approval"})
        # if the current user is not the director of the employee
        else:
            total = self.total_amount_to_extend()
            if total == 0.0:
                approval_message = f"""You have a Budget {self.name} for {self.user_id.name} to Approve.'"""
            else:
                approval_message = f"""You have a request for extend budget for Budget {self.name} to Approve."""
            self._send_budget_notification(
                subject="Budget : Approval Notification",
                message=approval_message,
                user_name=self.user_id.emp_id.director_id.login,
                user_email=self.user_id.emp_id.director_id.name
            )
            self.write({'expense_state': 'gm_approval'})
        ...
        if accounting_users:
            ...
            for user_id in user_ids:
                ...
                self._send_budget_notification(
                    ...
                    message=message,
                    user_name=res_user.name,
                    user_email=res_user.emp_id.work_email
                )
    ...
    def action_extend_budget(self):
        ...
        budget_form_view = self.env.ref(
            'custom_expense_budget_approval.crossovered_budget_view_extend')
