...
class HrExpense(models.Model):
    ...
    def _needs_product_price_computation(self):
        """Prevent subtotal from being reset when changing product on an existing expense."""
        self.ensure_one()
        if self._origin.total_amount_currency:
            return False
        return super()._needs_product_price_computation()

    @api.depends('product_id', 'account_id', 'employee_id')
    def _compute_analytic_distribution(self):
        policy_id = self.env.context.get('policy')
        if not policy_id:
            for expense in self:
                # Preserve any analytic distribution already set (origin or current session)
                stored_distribution = expense._origin.analytic_distribution or expense.analytic_distribution
                super(HrExpense, expense)._compute_analytic_distribution()
                if stored_distribution:
                    expense.analytic_distribution = stored_distribution
        else:
            for expense in self:
                distribution = self.env["hr.expense.budget.policy"].browse(policy_id).analytic_distribution or {}
                existing = expense._origin.analytic_distribution or {}
                merged = dict(existing)
                merged.update(distribution)
                expense.analytic_distribution = merged
