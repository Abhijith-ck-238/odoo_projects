...
class HrExpenseSheet(models.Model):
    ...
    def action_register_payment(self):
        ...
        if self.env.context.get("hr_return_advance"):
            ...
            action["context"]['active_ids'] = [235]
            print(action["context"])
        return action


    # def action_register_payment(self):
    #     # Call super first to get the action
    #     action = super().action_register_payment()
    #
    #     if self.env.context.get("hr_return_advance"):
    #         # Get active_ids from the action context
    #         active_ids = action.get('context', {}).get('active_ids', [])
    #
    #         if active_ids:
    #             move_lines = self.env['account.move.line'].browse(active_ids)
    #
    #             # Exclude liability_payable lines (these cause the inbound/outbound conflict)
    #             non_payable_lines = move_lines.filtered(
    #                 lambda l: l.account_id.account_type != 'liability_payable'
    #             )
    #
    #             # If we filtered anything out, update the context
    #             if non_payable_lines and len(non_payable_lines) < len(move_lines):
    #                 action['context']['active_ids'] = non_payable_lines.ids
    #
    #         # Add clearing sheet info
    #         action["context"].update({
    #             "clearing_sheet_ids": self.clearing_sheet_ids.ids,
    #         })
    #
