...
class DeliveryCarrier(models.Model):
    ...
    def sendcloud_send_shipping(self, pickings):
        ...
        :return dict: {'exact_price': a float,
        ...
        shipping_values = {}
        parcel_items_list = []
        shipping_values['name'] = pickings.partner_id.name
        if pickings.partner_id.company_name:
            shipping_values["company_name"] = pickings.partner_id.company_name
        else:
            shipping_values["company_name"] = pickings.partner_id.name
        shipping_values["email"] = pickings.partner_id.email
        shipping_values["telephone"] = pickings.partner_id.phone
        shipping_values["address"] = pickings.partner_id.street
        shipping_values["house_number"] = 0
        shipping_values["city"] = pickings.partner_id.city
        shipping_values["country"] = pickings.partner_id.country_id.code
        shipping_values["to_state"] = pickings.partner_id.state_id.code
        shipping_values["postal_code"] = pickings.partner_id.zip
        ...
        try:
            ...
            shipping_rate = self.sendcloud_rate_shipment(pickings.sale_id)
            tracking = response.json()['parcel']['tracking_number']
            pickings.sendcloud_parcel_no = response.json()['parcel']['id']
