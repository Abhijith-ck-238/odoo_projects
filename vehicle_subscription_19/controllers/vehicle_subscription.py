...
class OnlineSubscription(http.Controller):
    ...
    @http.route(['/online/subscription/city'], type='jsonrpc', auth="public",
                website=True)
    ...
    def vehicle_form(self, **kw):
        ...
        if kw.get('start_date'):
            ...
            vehicle = request.env['fleet.vehicle'].sudo().search(
                [('id', 'in', vehicle_ids.ids),('seats','>=',seating_capacity)])
    ...
    @http.route(['/online/subscription/book'], type='jsonrpc', auth="public",
                website=True)
    def get_vehicle(self, **kwargs):
        ...
        monthly_subscription = kwargs.get('monthly_subscription')
        ...
        subscribe = request.env['fleet.subscription'].sudo().create({
            ...
            'extra_km': km,
            'is_monthly_subscription' : monthly_subscription,
    ...
    @http.route(['/online/subscription/with/fuel'], type='jsonrpc', auth="public",
                website=True)
    def get_with_fuel(self, **kwargs):
        """Calculate price for vehicle according to fuel type """
        vehicle = int(kwargs.get('vehicle'))
        km = kwargs.get('extra_km') or 0
    ...
    @http.route(['/online/subscription/without/fuel'], type='jsonrpc',
                auth="public",
                website=True)
    ...
    @http.route('/online/choose/vehicle', type='jsonrpc', auth="public",
                website=True)
