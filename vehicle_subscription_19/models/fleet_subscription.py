...
class VehicleSubscription(models.Model):
    ...
    is_monthly_subscription = fields.Boolean(string="Monthly Subscription")
    ...
    default_km = fields.Float(string="Default KMS",
                              ...
                              help="Default km is set based on free km of "
                                   ...
                                   "person")
    extra_km = fields.Float(string="Extra KMS", default=1.0,
    ...
    def action_invoice(self):
        ...
        payment_term = self.env.ref("vehicle_subscription.account_payment_term_monthly_payment").id if self.is_monthly_subscription else False
    ...
    def action_cancel(self):
        ...
        all_invoicee = self.env['account.move'].search(
            [('id', 'in', self.invoice_ids.ids),])
             # ('payment_state', 'in', ['paid', 'partial'])])
        for i in all_invoicee:
            all_invoicee_1 = i
        invoicee = all_invoicee.filtered(lambda invoice: invoice.payment_state in ['paid', 'partial'])
        invoice = invoicee.mapped(
